...
class JobDataClass:
    ...
    def __init__(self, wind):
        ...
        self.window.title(app_name + "-Jobs")
        ww = 1300
        ...
        self.main_heading = Label(self.window, text='Job Details', fg=heading_fg, bg=heading_bg,
                                  font=('Calibri', 25, 'bold'), borderwidth=5, relief='groove')
        self.main_heading.place(x=0, y=0, relwidth=1, height=55)

        # -----------------------------------------creating table-------------------------------------

        #	department job_id job_title vacancies last_date




        self.table = Treeview(self.window, columns=['c1', 'c2', 'c3', 'c4', 'c5'],
                              height=25)
        self.table.heading('c1', text='Department')
        self.table.heading('c2', text='Job Id')
        self.table.heading('c3', text='Job Title')
        self.table.heading('c4', text='Vacancies')
        self.table.heading('c5', text='Last Date')

        self.table['show'] = 'headings'
        self.table.column('c1', width=160, anchor='center')
        self.table.column('c2', width=160, anchor='center')
        self.table.column('c3', width=160, anchor='center')
        self.table.column('c4', width=160, anchor='center')
        self.table.column('c5', width=160, anchor='center')


        # ------------------------------------------placing Table---------------------------------
        alignment = ((ww // 1.3) - (ww // 1.3) + 140) / 2
        self.table.place(x=ww//5, y=100, height=350)
        ...
        wid_y = 550
        diff_x = 130
        self.l1 = Label(self.window, text="Search by job title : ", fg=lab_col_fg,bg=window_bg, font=wid_font,anchor="w")
        ...
        self.clear1 = Label(self.window, text="clear", fg=lab_col_fg,font=wid_font, bg=window_bg)
        ...
        self.clear2 = Label(self.window, text="clear",fg=lab_col_fg, font=wid_font, bg=window_bg)
        ...
        self.b1 = Button(self.window, text='Jobs', fg=b_fg, bg=self.b_bg, font=wid_font,command=self.get_data_by_job)
        ...
        self.l1.place(x=alignment-5, y=wid_y, height=wid_h)
        alignment += diff_x+5
    ...
    def get_data_by_job(self):
        try:
            query = "select * from addjobs where job_title = %s "
    ...
    def get_data_by_department(self):
        try:
            query = "select * from addjobs where department = %s order by job_id asc"
    ...
    def print_out(self):
        ...
        import os
        pdf = PrintClass()
        headings = ["Department", "Job Id", "Job Title", "Vacancies", "Last Date"]
        new_data = []
        count = 0
        for row in self.data:
            new_data.append([self.data[count][0], self.data[count][1], self.data[count][2], self.data[count][3], self.data[count][4]])
            count+=1
        widths = [38,38,38,38,38]
        pdf.body(headings, new_data, widths,0)
        pdf.output("JobData.pdf")
        os.system('explorer.exe "JobData.pdf"')
