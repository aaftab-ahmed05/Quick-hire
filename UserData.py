...
class UserDataClass:
    ...
    def __init__(self, wind):
        ...
        self.main_heading = Label(self.window, text='User Data', fg=heading_fg, bg=heading_bg,
        ...
        self.table = Treeview(self.window, columns=['c1', 'c2'],
                              height=25)
        self.table.heading('c1', text='Username')
        self.table.heading('c2', text='User type')

        self.table['show'] = 'headings'
        self.table.column('c1', width=250, anchor='center')
        self.table.column('c2', width=250, anchor='center')
        ...
        self.table.place(x=ww//3.3, y=100, height=350)
        ...
        wid_y = 550
        diff_x = 130
        self.l1 = Label(self.window, text="Search by username : ", fg=lab_col_fg,bg=window_bg, font=wid_font,anchor="w")
        self.e1 = Entry(self.window, font=wid_font)
        self.l2 = Label(self.window, text="Search by user type : ", fg=lab_col_fg,bg=window_bg, font=wid_font, anchor="w")
        ...
        self.b1 = Button(self.window, text='Username', fg=b_fg, bg=self.b_bg, font=wid_font,command=self.get_data_by_username)
        self.b1.bind("<Enter>", self.b_enter)
        self.b1.bind("<Leave>", self.b_leave)
        self.b2 = Button(self.window, text='User type', fg=b_fg, bg=self.b_bg, font=wid_font,command=self.get_data_by_usertype)
        ...
        self.l1.place(x=alignment-10, y=wid_y, height=wid_h)
        alignment += diff_x+15
    ...
    def print_out(self):
        ...
        import os
        pdf = PrintClass()
        headings = ["Username", "User type"]
        new_data = []
        count = 0
        for row in self.data:
            new_data.append([self.data[count][0], self.data[count][2]])
            count+=1
        widths = [95,95]
        pdf.body(headings, new_data, widths,2)
        pdf.output("UserData.pdf")
        os.system('explorer.exe "UserData.pdf"')
